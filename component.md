...
- 将一个完整项目在开发时根据业务分成一个个可以单独运行的组件进行开发，用以分离依赖，降低业务耦合和干扰，减少每个组件的编译时间，在打包的时候又可以统一成一个完整的项目app进行编译打包
...
7. 组件间的调用和通信
    ...
    - 使用2：组件通信
        ...
        3. 在另一处调用
            ...
            ```
    - 注意：
        - 如果`build.gradle`中以及有`arguments`参数，则应该这样添加
            ```java
            kapt {
                arguments {
