...
- [步骤](#步骤)
  - [1. 创建组件化开关](#1-创建组件化开关)
  - [2. 创建组件化结构](#2-创建组件化结构)
  - [3. 为组件添加开关判断](#3-为组件添加开关判断)
  - [4. 解决AndroidManifest冲突](#4-解决androidmanifest冲突)
  - [5. 测试](#5-测试)
  - [6.为项目组件的单独开发增加application](#6为项目组件的单独开发增加application)
  - [7. 组件间的调用和通信](#7-组件间的调用和通信)
  - [8. 注意事项](#8-注意事项)
...
### 1. 创建组件化开关
- 组件化开关用以控制是否进行组件化编译。具体做法是在一个全局的位置设置一个可以控制的参数，组件将有此参数来判断自身应该作为`app`单独运行还是作为`lib`一起编译
- 方法1：可以直接在`gradle.properties`中添加`runAlone=false`
- 方法2：在项目模块中自建一个`gradle`文件然后声明
    ```java
    ext {
        runAlone = false
    }
...
- 第二种方法需要声明文件引入，在项目的`build.gradle`顶行加入
    ...
    如果文件有路径，需要添加相应路径
### 2. 创建组件化结构
- 一个组件化开发的项目包括一个`app`的module作为整体应用的壳，一个`common`的lib作为结构中心，以及若干依据功能分开的`lib`和依据业务分开的组件
- Project
    - `app`      (package)
    - `common`    (lib)
    - `lib1`      (lib)
...
### 3. 为组件添加开关判断
- 组件在分开开发时是一个application，而在组合应用时是一个lib，因此需要判断是`apply plugin: com.android.application`还是`apply plugin: com.android.library`
...
- 当设置完成后更改开关并同步项目，as中对应的组件图标会在`module`样式和`lib`样式中切换

### 6.为项目组件的单独开发增加application

- 这个操作是可选的，但一般来说都需要`application`
